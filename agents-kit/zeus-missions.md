...
- Note: the 12d fold-in from the series. Smallest correct change; hand every
  function to Heracles with its tests.

### M2: Hub Worker card (12f)
- Status: QUEUED
...
- Note: the 12f hub Worker card. Worker tone laws apply: gold never red, grade
  7, no guilt mechanics.

### M3: Dashboard deployment (13c)
- Status: QUEUED
- Lead: hermes, with athena for any build wiring
- Gates: Heracles green, Argus clean, canon consistency
- Human gate: none expected.
- Note: the 13c dashboard deployment. Direct push to main, smoke check on
  deploy, rollback before diagnosis if the smoke check fails.

### M4: Demo edge function (12a)
- Status: QUEUED
- Lead: athena
- Gates: Heracles green, Argus clean, canon consistency
- Human gate: if the function reads or writes schema, stop for Gary.
- Note: the 12a demo edge function. Three-layer resilience is mandatory on the
