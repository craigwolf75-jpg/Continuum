...
**The journey is the product: satisfied with one caveat.** All portals render the same canon (Prompt 17 enforced it mechanically: one Worker 15, one Worker 08, per-tenant counts that sum), and the bridge makes one event appear as role-appropriate projections across four surfaces at once, which is the brief's different views of the same connected journey made literal. The caveat: the journey is not yet visible as a single chronological record anywhere; each portal shows its slice, but the Recovery Journey Record as one timeline exists only in the worker's milestone view. That is a blueprint-phase build, noted as such.
