...
function onboardTo(mod, a, upTo) {
  ...
  const steps = [
    ...
    ["contacts", { coordinatorName: "Worker 59", coordinatorEmail: "k.morgan@example.com", smsDefaultsConfirmed: true }]
  ];
...
}
{
  ...
  mod.submit("contacts", { coordinatorName: "Worker 59", coordinatorEmail: "k.morgan@example.com" });
  const done = mod.finish();
...
// ---- test 18: wall and shape hardening on the question steps ----
{
  const { mod } = load();
  const a = fullAdapter(); mod.install(a); a._accept("ts");
  const res = mod.submit("contacts", { coordinatorName: "Worker 59", coordinatorEmail: "k.morgan@example.com", diagnosis_notes: "sneaks" });
