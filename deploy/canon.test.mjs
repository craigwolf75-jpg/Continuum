/* Continuum canon suite (Prompt B2). node deploy/canon.test.mjs
   ...
   Worker 15 day 9 pain 4, Worker 08 off work day 18, per-tenant numbers that
...
// -- Worker 15: day 9, pain 4 --
ok("worker: Worker 15 at day 9", day(worker, 9));
ok("worker: pain reaches 4", /pain:\s*4\b/.test(worker));
ok("hse: a case at day 9 (Worker 15)", day(hse, 9));
ok("clinical: Worker 15 present at day 9", clinical.includes("Worker 15") && day(clinical, 9));

// -- Worker 08: off work, day 18 --
ok("hse: Worker 08 off work at day 18", hse.includes("Worker 08") && day(hse, 18) && hse.includes("off_work"));
ok("clinical: Worker 08 at day 18", clinical.includes("Worker 08") && day(clinical, 18));
...
// -- Worker 15 prognosis is 21 days (canon), so day 21 is expected, not drift --
