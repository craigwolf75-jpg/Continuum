...
-- in the platform owned and platform retrofitted schemas that lacks a covering index. A foreign
-- key with no index on its referencing columns forces a sequential scan on the child table when
-- the parent row is updated or deleted, and denies the planner an index path for joins in that
-- direction. The fix is a plain btree index whose leading columns are the foreign key columns in
-- their constraint order.
--
-- Two groups are covered: the tenancy retrofit keys added by migrations 0010 to 0014
-- (organisation_id on the clinical, employer, audit, config and events tables, location_id on
-- clinic and wcb_report, and the wcb_report amendment chain and consent supersession keys), and
-- pre existing physician stream keys from clinical/db that shipped without a covering index
-- (worker_id, practitioner_id, report_id, code, measurement_id, previous_submission_id, and the
-- events outbox event_id). Indexing them is purely additive and non breaking.
--
