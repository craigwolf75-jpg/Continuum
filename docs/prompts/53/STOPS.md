...
When Gary pastes the file, G1 is read-only. Fix nothing found. If any
clinical content is reachable from an employer-facing surface, report
that finding under Section 5 before the rest finishes.
...
Craig confirmed the clinic and enterprise file used for shipped Prompt
...
## 50a Decision 2 stands

Schema scoping still stands. Platform schemas versus hub, site, demo,
and worker tables. Exclusion by schema. One-entry `mpi.person`
allow-list.
...
## Do not unwind local and CI work

The hold arrived in time. Nothing live to unwind. Montreal was never
created. Bedrock go was drafted but not sent. Nothing has been seeded.
...
## Master Architecture number 48

Number 48 remains reserved. On arrival it must be RREV 2 sent 12:34 PM.
The upload is checked against that stamp before any rewrite begins. Do
not invent Master Architecture text.

---

## Programme is not complete

More prompts exist that have not been sent. Do not treat the current
set as the complete programme.
