...
## Canonical versions (old stream)

These are the versions that stand. Earlier sends are discarded.

- Old Prompt 44 is RREV 2, sent 12:34 PM.
- Old Prompt 45 is REV 2, sent 12:32 PM.
- Old Prompts 46 through 49 are single versions.

Do not work from a discarded send.

---

## Classification

Old Prompts 44 to 46 are architecture reading, not building.

Old Prompts 47 to 49 are build prompts. They halt on their own
prerequisite checks by design. That halt is intended behaviour, not
failure.
...
3. HOLD occupational or reference seeding beyond SYNTH-prefixed test
   fixtures.
...
## Stands: 50a Decision 2

50a Decision 2 schema scoping still stands, and stands taller.

- Platform schemas versus hub, site, demo, and worker tables.
- Exclusion by schema.
- One-entry `mpi.person` allow-list.

Do not create `mpi.person`. Do not expand the allow-list.
...
Craig answered: the Prompt 45 clinic and enterprise file used for
shipped Prompt 47 was not REV 2 from 12:32 PM.
...
Number 48 is reserved for Master Architecture.

On arrival it must be RREV 2 sent 12:34 PM. The upload is checked
against that stamp before any rewrite begins.

Do not invent Master Architecture text. Do not treat a different send as
the arrival.
...
More prompts exist that have not been sent. The current set is not the
complete programme. Do not treat yesterday's set, or the current set, as
complete.
...
- Bedrock go was drafted but not sent.
- Nothing has ever been seeded.

What exists is local and CI work: Claude Code's pure-code build of
Prompts 39 to 44 (production submission structurally disabled, no model
...
The file is not in hand. Do not invent G1. Do not start G1.

When it runs, two governing rules:

1. Read-only. Fix nothing found.
2. Any clinical content reachable from an employer-facing surface is
   reported immediately under Section 5, before the rest finishes.

Fix nothing. Section 5 first if a clinical leak is reachable from an
employer surface.
...
- 53 is the next free number after 52.
- 48 remains reserved for Master Architecture (RREV 2 required on
  arrival).
- 49 is unassigned.
