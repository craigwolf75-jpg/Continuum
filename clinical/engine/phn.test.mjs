...
import { ALBERTA_JURISDICTION } from "../db/provincial_rules.data.mjs";

let pass = 0, fail = 0;
const ok = (n, c) => { if (c) pass++; else { fail++; console.error("  FAIL: " + n); } };

const AB = {
  worker_identifier_label: ALBERTA_JURISDICTION.worker_identifier_label,
  worker_identifier_pattern: ALBERTA_JURISDICTION.worker_identifier_pattern,
};

// -- stage 1: length (criterion 8), pattern from jurisdiction data --
ok("a missing profile fails named, never defaults a pattern", phnLength("123456789").some((f) => f.id === "PHN-PROFILE"));
ok("a nine digit PHN passes the length check", phnLength("123456789", AB).length === 0);
ok("an eight digit PHN is rejected by application code (criterion 8)", phnLength("12345678", AB).length === 1);
ok("a ten digit PHN is rejected", phnLength("1234567890", AB).length === 1);
ok("a PHN with a non digit is rejected", phnLength("12345678X", AB).length === 1);
ok("a blank PHN is not a length failure (valX01 owns presence)", phnLength("", AB).length === 0);
ok("surrounding whitespace is trimmed then checked", phnLength("  123456789  ", AB).length === 0);
...
// -- the full gate --
ok("gate: a valid nine digit PHN with check digit off passes", phnGate("123456789", AB).length === 0);
ok("gate: an eight digit PHN fails on length and never reaches the check digit", (() => { let called = false; const f = phnGate("12345678", { ...AB, enabled: true, validator: () => { called = true; return true; } }); return f.length === 1 && f[0].id === "PHN-LENGTH" && called === false; })());
ok("gate: a nine digit PHN failing an enabled check digit is rejected", phnGate("123456780", { ...AB, enabled: true, validator: () => false }).length === 1);
