/* Prompt 47 Part 2: clinic onboarding product half.
   ...
   blocking gate (Part 2.6 as restated in the mission): privacy pack, IMA,
   admin MFA, sandbox test, and a valid practitioner contract and role pair.
   A privacy pack override requires a recorded reason and a persistent banner.
...
export function readinessGate(run, store) {
  requireStore(store, "ONBOARDING-STORE-MISSING", "An onboarding store is required.");
...
export function goLive(run, store, at) {
  const gate = readinessGate(run, store);
  if (!gate.ready) {
  ...
  run.status = "live";
  run.go_live_at = at || (store && store.now) || new Date().toISOString();
  return run;
}
