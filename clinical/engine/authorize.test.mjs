...
import { resolvePermission, assertDelegation, emergencyElevationAllowed, ENTERPRISE_ROLES, CONTINUUM_ROLES } from "./authorize.mjs";

let pass = 0, fail = 0;
const ok = (n, c) => { if (c) pass++; else { fail++; console.error("  FAIL: " + n); } };
const throws = (fn, code) => { try { fn(); return false; } catch (e) { return !code || e.code === code; } };

ok("enterprise roles are data", ENTERPRISE_ROLES.reception.hard === "no_clinical" && ENTERPRISE_ROLES.clinic_auditor.read_only === true);
...
ok("clinic auditor is read only", resolvePermission("clinic_auditor", { type: "organisation", id: "org-1" }, null, {
